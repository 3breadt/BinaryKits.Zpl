...
public abstract class BarcodeDrawerBase : ElementDrawerBase
{
        ...
        DrawerOptions options)
    {
        using (new SKAutoCanvasRestore(this._skCanvas))
        {
            ...
            skFont.MeasureText(interpretation, out var textBounds, skPaint);
            ...
            {
                this._skCanvas.DrawShapedText(interpretation, x, y - margin, SKTextAlign.Left, skFont, skPaint);
            }
            else
            {
                this._skCanvas.DrawShapedText(interpretation, x, y + barcodeHeight + textBounds.Height + margin, SKTextAlign.Left, skFont, skPaint);
            }
        }
    }
    ...
    protected SKBitmap BoolArrayToSKBitmap(bool[] array, int height, int moduleWidth = 1)
    {
        ...
        return image.Resize(new SKSizeI(image.Width * moduleWidth, height), SKSamplingOptions.Default);
    }
    ...
    protected SKBitmap BitMatrixToSKBitmap(BitMatrix matrix, int pixelScale)
    {
        ...
        return image.Resize(new SKSizeI(image.Width * pixelScale, image.Height * pixelScale), SKSamplingOptions.Default);
    }
