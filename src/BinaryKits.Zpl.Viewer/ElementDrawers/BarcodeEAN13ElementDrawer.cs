        ...
        DrawerOptions options)
    {
        using (new SKAutoCanvasRestore(this._skCanvas))
        {
            ...
            for (int i = 0; i < interpretation.Length; i++)
            {
                ...
                skFont.MeasureText(digit, out var digitBounds, skPaint);
                this._skCanvas.DrawText(digit, x - (spacing + digitBounds.Width) / 2 - moduleWidth, y + barcodeHeight + textBounds.Height + margin, SKTextAlign.Left, skFont, skPaint);
                x += spacing;
                if (i == 0 || i == 6)
                {
                    x += moduleWidth * 4;
